...
async function currentUser(req, _res, next) {
  ...
  const result = await query("SELECT id, email, role, created_at, locked FROM users WHERE id = $1", [
    session.userId
  ]);
...
app.post("/api/auth/logout", async (req, res) => {
  ...
  return res.json({ message: "Logout facut doar in browser; token-ul vechi ramane reutilizabil." });
});
...
app.post("/api/auth/forgot-password", async (req, res) => {
  ...
  await query("INSERT INTO reset_tokens (user_id, token) VALUES ($1, $2)", [user.id, token]);
  await logAudit({
    userId: user.id,
...
app.post("/api/auth/reset-password", async (req, res) => {
  ...
  await query("UPDATE users SET password_hash = $1 WHERE id = $2", [newPassword, reset.user_id]);
  await logAudit({
